...
import { authResponseSchema, type AuthResponse, type LoginInput, type RegisterInput } from "@/src/server/validators/auth";
...
export class AuthService {
  ...
  async login(input: LoginInput): Promise<Either<AuthError, AuthResponse>> {
    ...
    if (!user || !user.isActive || !comparePassword(input.password, user.passwordHash)) {
